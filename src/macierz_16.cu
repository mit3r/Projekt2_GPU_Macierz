...
int MatrixMultiply(int n, const dim3& dimsA, const dim3& dimsB,
  ...
  for (int j = 0; j < nIter; j++) {
    switch (n) {
      ...
      DISPATCH_KERNEL(BLOCK_SIZE, 18);
      DISPATCH_KERNEL(BLOCK_SIZE, 19);
      DISPATCH_KERNEL(BLOCK_SIZE, 20);
      DISPATCH_KERNEL(BLOCK_SIZE, 21);
      DISPATCH_KERNEL(BLOCK_SIZE, 22);
      DISPATCH_KERNEL(BLOCK_SIZE, 23);
  ...
  checkCudaErrors(
      cudaMemcpyAsync(h_C_out, d_C, mem_size_C, cudaMemcpyDeviceToHost, stream));
  checkCudaErrors(cudaStreamSynchronize(stream));
...
int main(int argc, char** argv) {
  ...
  int matrix_result = MatrixMultiply<block_size>(n, dimsA, dimsB, h_A, h_B, h_C);
  bool test1_ok = true;
  ...
  int matrix_result_second = MatrixMultiply<block_size>(n, dimsA, dimsB, h_A, h_B, h_C);
  bool test2_ok = true;
